...
def multilabel_sample(y, size=1000, min_count=5, seed=None):
    ...
    rng = np.random.RandomState(seed)  # let np.RandomState(seed=None) assumes default seed from the /dev/urandom or clock
...
def multilabel_train_test_split(X, Y, size, min_count=5, seed=None):
    ...
    # train_set_idxs = np.setdiff1d(index, test_set_idxs) # not used
...
def sample_report(y, y_sample, figsize=(16, 32)):
    ...
    ratio = ratio.reset_index(drop=True).sort_values('sample_ratio', ascending=True, na_position='first')
    ratio.plot(kind='barh', stacked=True, figsize=figsize)
...
def split_report(y, y_train, y_test, figsize=(10, 10)):
    ...
    # ratio.index.name = 'label__class'
    ratio = ratio.reset_index(drop=True).sort_values(['train_ratio', 'test_ratio'], ascending=True, na_position='first')
    ratio.plot(kind='barh', stacked=True, figsize=figsize)
...
def sample_split_report(y, y_sample, y_train, y_test, figsize=(10, 10)):
    ...
    ratio = pd.concat({'sample_ratio': y_sample.value_counts() / y_counts,
                       ...
                       'test_ratio': y_test.value_counts() / y_counts}, axis=1)
    ratio.index.name = 'label__class'
    ratio = ratio.sort_values(['sample_ratio', 'train_ratio', 'test_ratio'], ascending=True, na_position='first')
    ratio.plot(kind='barh', stacked=True, figsize=figsize)
    sample_size = y_sample.shape[0] / y.shape[0]
